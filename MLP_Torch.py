...
full_train_dataset = MnistDataset("C:/python/pythonProject/deep_learning/mnist_numpy/train", transform=transform, flatten=True)
train_size = int(0.8 * len(full_train_dataset))
val_size = len(full_train_dataset) - train_size
train_dataset, val_dataset = torch.utils.data.random_split(full_train_dataset, [train_size, val_size])
test_dataset = MnistDataset("C:/python/pythonProject/deep_learning/mnist_numpy/test", transform=transform, flatten=True)
...
model = SimpleNet(n_input, n_hidden)
...
for epoch in range(epochs):
    ...
    if epoch % 1 == 0:
        ...
        print(f"Epoch {epoch + 1}, Loss: {loss:.4f}, train_acc: {train_acc:.2f}, val_acc: {val_acc}")
